"""Parametric Cheney Nat 4% U=1 passive-lset test — sweep ignition
duration + canopy drag to isolate the low-U reversal driver.

CLI: _cheney_low_U_param_test_worker.py LABEL IGNITION_DUR_S CANOPY_C_D OUT_DIR
"""
import os
...
def main():
    ...
    print(f"\n=== Cheney Nat 4% U=1  {label}  "
          f"ign_dur={ign_dur}s  C_d={c_d}  sim_t={SIM_T}s ===", flush=True)
    t0 = time.time()
    r = run_3d_spread(
        ri, wind_speed_m_s=U,
        ...
        ignition_T_pin_ramp_s=min(0.5, ign_dur / 4.0),  # shorter ramp for short pin
        min_dt_s=1.0e-4,
    ...
    for sp in snaps:
        ...
        if col_burned.any():
            ...
            ts_x.append(float(s["x_mid"][np.where(col_burned)[0].max()]))
    if len(ts_t) >= 3:
        mask = [i for i, t in enumerate(ts_t) if t >= max(2.0, ign_dur + 0.5)]
        if len(mask) >= 3:
    ...
    result = dict(
        label=label, ign_dur_s=ign_dur, canopy_C_d=c_d,
    ...
    print(f"\n[done] {label}  ROS_Ts={ros_Ts:.2f} m/min  "
